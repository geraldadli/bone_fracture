...
<script>(() => {
    ...
    const start = () => {
        const video = document.querySelector('.st-key-trailer video');
        if (!video) { if (Date.now() < deadline) setTimeout(start, 200); return; }
        if (!video.played.length) { video.playsInline = true; video.play().catch(() => {}); }
    };
    start();
